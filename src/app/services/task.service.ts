...
import { ExtendedTask, NotFoundError, TaskHour, TaskWorkSpent } from "@lib";
...
export class TaskService {
    ...
    public getProjectTasks(projectId: string, limit: number, offset: number): Observable<EntityList<ExtendedTask>> {
        const url = `${this.apiUrl}/projects/${projectId}/tasks`;
        const params = {
            limit,
            offset,
        };
        return this.http.get(url, { params, observe: "response" }).pipe(
            mapToType<HttpResponse<ExtendedTask[]>>(),
            map((resp: HttpResponse<ExtendedTask[]>) => {
